...
| # | Phase | Detailed PRD | Depends on | Status | Progress |
|---|---|---|---|---|---|
| 46 | Async friend duels (seeded share-a-run challenge) | [phase-46-async-friend-duels.md](phases/phase-46-async-friend-duels.md) | 2, 15, 42, 44, 45, 6 | 🚧 Built — awaiting review/merge | v1 score duel **built + green** on `worktree-phase-46-async-friend-duels` (2026-07-18). Every run is seeded (reproducible); pure codec + `duelScore`/`duelVerdict`; Summary "Duel a friend" (gated ≥ 10 + filter-based) with link/code/PNG + name prompt; `#/duel` route for the received challenge + return verdict + broken-link; head-to-head verdict → send-result → rematch; `playerName` in prefs/Settings; static OG card (`og-card.png`) **and** in-app PNG; EN/FR/DE. check/lint clean, prod build ships the card, headless screenshots verified. **46b (2026-07-20) — the Grandmaster "become a grandmaster too" invite — now built too:** separate `grandmaster-invite.ts` codec + `#/challenge-invite` route with a LOCKED state (unmastered receiver), COOLDOWN & ACCEPT; challenger entry on the victory bloom + each certified Progress capstone → a slide-up **share sheet** (inline arena/ember card + Share / Copy image / Copy link / Show QR); scan-to-play QR; silent return (OQ7); EN/FR/DE. **923 tests**, check/lint clean. Not yet merged to main. |
