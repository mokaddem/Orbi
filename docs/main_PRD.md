...
| # | Phase | Detailed PRD | Depends on | Status | Progress |
|---|---|---|---|---|---|
| 44 | Mastery Challenge (capstone / "prove-it" run) | [phase-44-mastery-challenge.md](phases/archive/phase-44-mastery-challenge.md) | 41, 16, 2, 35, 5 | ✅ Done | Functional mechanic + audio feature-complete (**"Grandmaster Challenge"**: one-life Gauntlet, both directions, continents only). Domain + store (unlock/queue/grading), 15 monotonic capstones + `challenge.*` EN/FR/DE, the `/challenge` shell + pickers, the Progress reward (prove-it/gilded/prestige), **and the 5-cue escalating-bed audio** (`0974d99`+`fe728d9`). 777 tests. **Presentation (cinematic UI, no-XP/no-Summary flow, daily cooldown) split to Phase 45.** Merged to main + archived (818 tests). |
| 45 | Grandmaster Challenge — cinematic UI + no-XP flow + cooldown | [phase-45-grandmaster-cinematic-ui.md](phases/archive/phase-45-grandmaster-cinematic-ui.md) | 44, 41, 16, 6, 33 | ✅ Done | The locked dark-teal arena ([`gauntlet-ui-spec.md`](gauntlet-ui-spec.md)) — **all sub-phases ①–⑥ feature-complete, merged to main**. ①–④: `--g-*` theme + `GrandmasterCrest`; the arena HUD (tier sidebar + embers + heat vignette + beating life); the **offer modal** (dynamic slot count) + **cinematic entry**; the **in-arena end screens** (golden **victory bloom** + **runover**, off `/summary`). **⑤**: a dedicated **`grandmaster` IDB store** (DB v6) holds per-`family\|region` certification + the daily cooldown; the finish writes there (**not** `saveSession`) so a run grants **zero XP / stats / streak**; the 15 capstones moved **off** the achievements/XP path (XP-neutral) — gilding, the "Grandmaster X/15" prestige, and a "Certified!" toast all read the store; and a **once-a-day-per-family×region cooldown** (resets local midnight) gates the entry cell (clock affordance) + the modal (countdown, Accept removed). **⑥ landed**: a **Home-screen invitation card** (`GrandmasterInviteCard`) — shown only when ≥ 1 run is attemptable today (a pure `availableChallenges`); one available → the offer modal on Home → the arena; more → a `focusMastery` handoff that spotlights Progress's World Mastery panel (Progress unchanged). The shared offer modal also shows a rough **~N-min duration estimate** (a pure `estimateChallengeMinutes`) beside the question/life stakes. 798 tests, check/lint clean; headless drives confirm the invite's three branches (single→modal→arena, multi→/progress spotlight, hidden) + the modal's duration cell. Audio already shipped (Phase 44). Merged to main + archived (818 tests). |
...
| # | Phase | Detailed PRD | Depends on | Status | Progress |
|---|---|---|---|---|---|
| 46 | Async friend duels (seeded share-a-run challenge) | [phase-46-async-friend-duels.md](phases/archive/phase-46-async-friend-duels.md) | 2, 15, 42, 44, 45, 6 | ✅ Done | **Shipped in v2.6.0 "Challenge a friend" (2026-07-20).** v1 score duel **built + green** on `worktree-phase-46-async-friend-duels` (2026-07-18). Every run is seeded (reproducible); pure codec + `duelScore`/`duelVerdict`; Summary "Duel a friend" (gated ≥ 10 + filter-based) with link/code/PNG + name prompt; `#/duel` route for the received challenge + return verdict + broken-link; head-to-head verdict → send-result → rematch; `playerName` in prefs/Settings; static OG card (`og-card.png`) **and** in-app PNG; EN/FR/DE. check/lint clean, prod build ships the card, headless screenshots verified. **46b (2026-07-20) — the Grandmaster "become a grandmaster too" invite — now built too:** separate `grandmaster-invite.ts` codec + `#/challenge-invite` route with a LOCKED state (unmastered receiver), COOLDOWN & ACCEPT; challenger entry on the victory bloom + each certified Progress capstone → a slide-up **share sheet** (inline arena/ember card + Share / Copy image / Copy link / Show QR); scan-to-play QR; silent return (OQ7); EN/FR/DE. Merged with latest `main` and released as **v2.6.0** — **938 tests** on the combined tree, check/lint clean. |
